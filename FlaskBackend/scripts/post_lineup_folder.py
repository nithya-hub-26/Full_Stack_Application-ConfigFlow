...
LINEUP_FOLDER_JSON_FILE = "post_folder.json"

def post_lineup_folder():
    logging.info("Starting the post_lineup_folders process.")
    url_path = get_url() + "/lineups/folders"
    headers = {
    ...
    try:
        ...
        if not new_folder_name:
            return {"status": "failure", "errorMessage": "Missing folder name"}, 400

        # Load the existing JSON data
        with open(LINEUP_FOLDER_JSON_FILE, "r") as file:
            data = json.load(file)
