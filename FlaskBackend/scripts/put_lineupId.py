...
JSON_FILE = "post_lineup.json"

def put_lineup():
    logging.info("Starting the put_lineups process.")
    try:
        # Get new lineup name from the frontend request
        request_data = request.json

        lineup_id = request_data.get("lineupId")
        new_lineup_name = request_data.get("lineup")

        if not lineup_id or not new_lineup_name:
            return {"status": "failure", "errorMessage": "Missing lineup ID or name"}, 400

        url_path = get_url() + f"/lineups/{lineup_id}"
        headers = {
        ...
        logging.info("Updated lineup name: %s", new_lineup_name)
    ...
    if response.status_code == 200:
        logging.info("Successfully updated the lineup name.")
        return {"status": "success", "message": "Lineup name is updated successfully"}
        ...
        logging.error("Lineup not found. Status: %d.", response.status_code)
        return {"status": "failure", "errorMessage": "Lineup not found."}
    else:
...
logging.info("Finished the put_lineups process.")
