...
JSON_FILE = "post_pool.json"

def put_pools():
    logging.info("Starting the put_pools process.")

    try:
        # Get new pool name from the frontend request
        request_data = request.json

        pool_id = request_data.get("id")
        new_pool_name = request_data.get("name")

        if not pool_id or not new_pool_name:
            return {"status": "failure", "errorMessage": "Missing pool ID or name"}, 400
        ...
        logging.info("Updated pool name: %s", new_pool_name)
    ...
    if response.status_code == 200:
        logging.info("Successfully updated the pool name.")
        return {"status": "success", "message": "Pool updated successfully"}
        ...
        logging.error("Pool not found. Status: %d.", response.status_code)
        return {"status": "failure", "errorMessage": "Pool not found."}
    elif response.status_code == 409:
        logging.error("Pool update failed. Status: %d. Reason: %s.",response.status_code, response.reason)
        return {"status": "failure", "errorMessage": "Update failed. Reason: " + response.reason}
    else:
        logging.error("Request failed. Status: %d. Reason: %s", response.status_code, response.reason)
        return {"status": "failure", "errorMessage": "Put request failed. Reason: " + response.reason}


logging.info("Finished the put_pools process.")