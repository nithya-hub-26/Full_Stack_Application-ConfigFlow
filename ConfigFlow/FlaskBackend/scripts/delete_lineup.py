...
def delete_lineup(lineup_id):
    logging.info(f"Starting delete process for lineup ID: {lineup_id}")
    ...
    try:
        ...
        if response.status_code == 200:
            logging.info("Lineup deleted successfully")
            return {"status": "success", "message": "Lineup deleted successfully"}
            ...
            logging.error(
            "Deletion failed. Status: %d. Reason: %s. The lineup has active configurations or is in use by another user.",
...
logging.info("Finished the delete lineup process.")
