...
def get_alarm_messages(lineupId):
    logging.info("Starting the get_alarm_messages process.")
    # Forming the request
    url_path = get_url() + "/messages"
    headers = {"accept": "application/json", "CustomToken": get_token()}
    params = {"limit": 10, "correlatedAlarmsForLineupId": lineupId, "sort": "-transactionId", "includeCorrelatedLineupIds": "false"}

    logging.info("Sending GET request to URL: %s", url_path)

    # Sending the request
    response = requests.get(url=url_path, params=params, headers=headers, verify=get_verify())

    if response.status_code == 200:
        logging.info("Request successful. Response status: %d", response.status_code)
        json_data = response.json()
        logging.info("Response JSON: \n%s", json.dumps(json_data, indent=4))
        return {"status": "success", "response": json_data.get("messages")}
...
logging.info("Finished the get_alarm_messages process.")
