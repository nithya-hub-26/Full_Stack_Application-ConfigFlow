...
def delete_lineup_config(config_id):
    logging.info(f"Starting delete process for config ID: {config_id}")
    url_path = get_url() + f"/configs/{config_id}"
    ...
    try:
        ...
        if response.status_code == 200:
            logging.info("Config deleted successfully")
            ...
            logging.error(
            "Deletion failed. Status: %d. Reason: %s. The config has active configurations or is in use by another user.",
...
logging.info("Finished the delete lineup config process.")
